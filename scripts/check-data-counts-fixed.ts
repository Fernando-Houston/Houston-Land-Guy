...
async function checkAllCounts() {
  ...
  try {
    const counts = await Promise.all([
      ...
      prisma.populationProjection.count().then(c => ({ name: 'Population Projections', count: c }))
    ])
    ...
    console.log('\n🏢 Employment & Economic:')
    counts.filter(c => ['Employers (DP5)', 'Economic Indicators (DP5)', 'Population Projections'].includes(c.name))
      .forEach(c => console.log(`  ${c.name}: ${c.count}`))
